...
fn spawn_grid(
    ...
    let atlas_grid = Tilemap::from_fn(width, height, |x, y| TextureAtlasTile::new(x + y * 4));
    ...
    commands.spawn(SpriteBundle {
        sprite: Sprite {
            color: Color::WHITE,
            custom_size: Some(
                vec2(width as f32 + 1.0, height as f32 + 1.0) * grid_geometry.tile_size,
            ),
            ..Default::default()
        },
        ..Default::default()
    ...
    let pos = -0.5 * grid_geometry.tile_size * vec2(width as f32 - 1.0, height as f32 - 1.0);
    for x in 0..width {
        for y in 0..height {
